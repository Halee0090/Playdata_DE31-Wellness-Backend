...
#import requests # requests 라이브러리는 FastAPI의 비동기 특성을 완전히 활용하지 못합니다. 
import httpx
...
async def get_kakao_token(request: Request):
    try:
        ...
        # 카카오 API에 POST 요청 보내기(request > client로 수정)
        ...
        if response.status_code == 200:
            token_data = await response.json()#response.json() 메서드도 비동기로 처리
            ...
            error_details = await response.json()#response.json() 메서드도 비동기로 처리
