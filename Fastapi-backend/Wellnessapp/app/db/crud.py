...
# 사용자의 마지막 업데이트 기록 조회
async def get_user_updated_at(db: AsyncSession, current_user: models.User):
    try:
...
async def get_recommend_by_user_id(db: AsyncSession, user_id: int):
    try:
        stmt = select(models.Recommend).filter(models.Recommend.user_id == user_id)
        result = await db.execute(stmt)
...
async def calculate_and_save_recommendation(db: AsyncSession, user: models.User):
...
async def get_or_update_recommendation(db: AsyncSession, current_user: models.User):
    try:
        # recommend 테이블에 기록 조회
        stmt = select(models.Recommend).filter(models.Recommend.user_id == current_user.id)
        result = await db.execute(stmt)
...
# 총 섭취량 조회
async def get_total_today(db: AsyncSession, current_user: models.User, date_obj: date):
    try:
        logger.info(f"Checking total_today for user: {current_user.id} on date: {date_obj}")
        stmt = select(models.Total_Today).filter(models.Total_Today.user_id == current_user.id)
        result = await db.execute(stmt)
        total_today = result.scalars().first()
        return total_today
...
# Total_Today 업데이트
async def update_total_today(db: AsyncSession, total_today: models.Total_Today):
    try:
        # 최대 허용 범위에 맞춰 값 제한
...
async def get_food_by_category(db: AsyncSession, category_id: int) -> Food_List:
    stmt = select(Food_List).filter(Food_List.category_id == category_id)
    result = await db.execute(stmt)
...
async def get_recommend_by_user(db: AsyncSession, current_user: models.User) -> Recommend:
     stmt = select(models.Recommend).filter(models.Recommend.user_id == current_user.id)
     result = await db.execute(stmt)
...
# history 저장 함수 
async def create_history(db: AsyncSession, current_user: models.User, category_id: int, meal_type_id: int, image_url: str, date: date):
...
# meals 조회 함수
async def get_meals_by_user_and_date(db: AsyncSession, current_user: models.User, date: datetime):
...
# 사용자 생성
async def create_user(db: AsyncSession, user: schemas.UserCreate, age: int, gender: int):
    db_user = models.User(
...
async def update_total_today_condition(db: AsyncSession, total_today_id: int, new_condition: bool):
    try:
        stmt = select(models.Total_Today).filter(models.Total_Today.id == total_today_id)
        result = await db.execute(stmt)
        total_today = result.scalar_one_or_none()
        
        if total_today:
            # 사용자 권장 영양소 가져오기
            stmt = select(models.Recommend).filter(models.Recommend.user_id == total_today.user_id)
            result = await db.execute(stmt)
            recommendation = result.scalar_one_or_none()
...
async def calculate_age(birth_date) -> int:
    today = date.today()
